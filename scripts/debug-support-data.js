...
const debugLogFile = path.resolve(__dirname, '..', 'support-debug.log');

// Create or clear the log file
fs.writeFileSync(debugLogFile, '--- Support Data Processing Debug Log ---\n\n');
...
appendToLog('Starting a local test server...');
...
server.stdout.on('data', (data) => {
  ...
  if (output.includes('support')) {
    appendToLog('SERVER: ' + output.trim());
  }
});

server.stderr.on('data', (data) => {
  appendToLog('SERVER ERROR: ' + data.toString().trim());
});
...
setTimeout(() => {
  ...
  testDateRange('month')
    ...
    .catch(error => {
      appendToLog('TEST ERROR: ' + error.message);
      server.kill();
      process.exit(1);
    });
...
async function testDateRange(range) {
  ...
  try {
    ...
    appendToLog(`Received ${data.length} CSAT data points`);
    
    // Analyze data
    if (data.length > 0) {
      const totalTickets = data.reduce((sum, item) => sum + item.totalTickets, 0);
      appendToLog(`Total tickets across all months: ${totalTickets}`);
      ...
      appendToLog(`Date range: ${oldestDate.toISOString().split('T')[0]} to ${newestDate.toISOString().split('T')[0]}`);
      
      // Show month-by-month breakdown
      appendToLog('\nMonthly breakdown:');
      data.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
          .forEach(item => {
            appendToLog(`- ${item.date}: ${item.totalTickets} tickets`);
          });
    }
    
    return true;
  } catch (error) {
    appendToLog(`Error testing ${range}: ${error.message}`);
    throw error;
  }
}
