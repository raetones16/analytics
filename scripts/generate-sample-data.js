...
xlsx.writeFile(
  webAppWorkbook,
  path.join(CUSTOMER_DIR, "Web App Stats (Last 90 Days).xlsx")
);
log(LOG_LEVEL.INFO, "Generated Web App Stats file");
...
xlsx.writeFile(
  mobileAppWorkbook,
  path.join(CUSTOMER_DIR, "Mobile App Stats (Last 90 Days).xlsx")
);
log(LOG_LEVEL.INFO, "Generated Mobile App Stats file");
...
xlsx.writeFile(
  timesheetWorkbook,
  path.join(CUSTOMER_DIR, "Timesheet Stats (Last 90 Days).xlsx")
);
log(LOG_LEVEL.INFO, "Generated Timesheet Stats file");
...
const salesforceCsv =
  ...
  salesforceData
    .map(
      (opp) => `${opp.Id},${opp.CloseDate},${opp.Amount},${opp.NumberOfModules}`
    )
    .join("\n");

fs.writeFileSync(
  path.join(SALES_DIR, "salesforce-won-opportunities.csv"),
  salesforceCsv
);
log(LOG_LEVEL.INFO, "Generated Salesforce Opportunities file");
...
xlsx.writeFile(
  supportWorkbook,
  path.join(SUPPORT_DIR, "Freshdesk Tickets 20250321_111939.xlsx")
);
log(LOG_LEVEL.INFO, "Generated Support Tickets file");

log(LOG_LEVEL.INFO, "Sample data generation complete!");
