...
const PRODUCT_CHARTS_LAYOUT_KEY = "product_charts_layout";
const getDefaultProductChartsLayout = (): {
  id: string;
  position: number;
  width: "half" | "full";
}[] => [
  { id: "userLogins", position: 0, width: "half" },
  { id: "absencesBooked", position: 1, width: "half" },
  { id: "timesheetsSubmitted", position: 2, width: "half" },
  { id: "workflowsCreated", position: 3, width: "half" },
];
...
export function ProductCharts({ data }: ProductChartsProps) {
  ...
  const hasSyntheticDistribution = data.some(
    (item) => item._synthetic?.distribution === true
  );
  ...
  if (hasSyntheticData) {
    syntheticDetails =
      "Using generated values because no real data was available";
    indicatorType = "full";
      ...
      >
        ...
        >
          <div className="flex justify-between items-center mb-3">
            ...
            <div
              className={`transition-opacity duration-200 ${
                hoveredChart === "userLogins" ? "opacity-100" : "opacity-0"
              }`}
            >
              <VisualizationToggle
          ...
          <UserLoginsChart
            data={data}
            visualizationType={
              visualPreferences.userLogins as "line" | "bar" | "area"
            }
          />
        </div>
        ...
        <div
          ...
          className="bg-white p-4 rounded-lg shadow-md relative"
          onMouseEnter={() =>
            !isEditingLayout && setHoveredChart("absencesBooked")
          }
          onMouseLeave={() => !isEditingLayout && setHoveredChart(null)}
        >
          <div className="flex justify-between items-center mb-3">
            ...
            <div
              className={`transition-opacity duration-200 ${
                hoveredChart === "absencesBooked" ? "opacity-100" : "opacity-0"
              }`}
            >
              <VisualizationToggle
          ...
          <AbsencesBookedChart
            data={data}
            visualizationType={
              visualPreferences.absencesBooked as "bar" | "line" | "stacked"
            }
          />
        </div>
        ...
        <div
          ...
          className="bg-white p-4 rounded-lg shadow-md relative"
          onMouseEnter={() =>
            !isEditingLayout && setHoveredChart("timesheetsSubmitted")
          }
          onMouseLeave={() => !isEditingLayout && setHoveredChart(null)}
        >
          <div className="flex justify-between items-center mb-3">
            ...
            <div
              className={`transition-opacity duration-200 ${
                hoveredChart === "timesheetsSubmitted"
                  ? "opacity-100"
                  : "opacity-0"
              }`}
            >
          ...
          <TimesheetsSubmittedChart
            data={data}
            visualizationType={
              visualPreferences.timesheetsSubmitted as
                | "line"
                | "bar"
                | "stacked"
            }
        ...
        <div
          ...
          className="bg-white p-4 rounded-lg shadow-md relative"
          onMouseEnter={() =>
            !isEditingLayout && setHoveredChart("workflowsCreated")
          }
          onMouseLeave={() => !isEditingLayout && setHoveredChart(null)}
        >
          <div className="flex justify-between items-center mb-3">
          ...
          <WorkflowsCreatedChart
            data={data}
            visualizationType={
              visualPreferences.workflowsCreated as "bar" | "line" | "area"
            }
          />
        </div>
      </LayoutManagerWithGrid>
