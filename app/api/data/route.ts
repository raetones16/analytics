  ...
  try {
    ...
    switch (type) {
      case 'product':
        ...
        log(LOG_LEVEL.INFO, `Returning ${productData.length} product data points`);
      ...
      case 'csat':
        const csatData = await processCSATData();
        log(LOG_LEVEL.INFO, `Returning ${csatData.length} CSAT data points`);
        return NextResponse.json(csatData);
      
      case 'all':
        log(LOG_LEVEL.INFO, 'Processing all data types...');
        const [allProductData, allSalesData, allCsatData] = await Promise.all([
          processProductData(),
          processSalesData(),
          processCSATData()
        ]);
        
        log(LOG_LEVEL.INFO, `Results - Product: ${allProductData.length} points, Sales: ${allSalesData.length} points, CSAT: ${allCsatData.length} points`);
        
        return NextResponse.json({
          productData: allProductData,
          salesData: allSalesData,
          csatData: allCsatData
        });
